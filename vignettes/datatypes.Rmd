...
## vectors en datatypes

Each variable in the `data.frame mydata` is a so called vector. A vector is a number of values that each have the same data type (Or mode^[`mode`, `storage.mode` and `type` are closely related concepts, we will not discuss the differences here. See also `?mode`.]). The most important data types in R are:^[There are a few more which we will not discuss.]: 
...
------------    ---------------------------------------------------    
  character:    text, for example 'man', 'woman', 'censored', etc. 
    ...
    numeric:    a numerical value like 1, 1.2 or 1e12 (that is 10 raised to the power of 12)
       list:    See below
-------------------------------------------------------------------
...
```{r}
lijst1 <- list("eleven", 12)
lijst2 <- list(c(1, 2, 3), c('foo', 'bar'))
```
We can also assign a name to the elements of a list:

```{r}
lijst3 <- list(getallen=c(1, 2, 3), chars=c('foo', 'bar'))
```
